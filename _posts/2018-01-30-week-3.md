...
2. **Local execution.**
    ...
    * **Social engineering** - trick or coerce the user into excuting the code themselves.
...
3. **Establish presense.**
   ...
   The malware is executing, but it doesn't have a permanent presense on the machine.
...
I want to take a moment to call out the person in class who suggested
...
The number one lesson here is that *defense in layers*.
...
note: a real av engine would unpack the file and
...
4. [Sample 4][sample4] copies a suspicious-looking executable to a temp directory and then deletes itself. Probably malicous.
...
system tray, with two options: US and CN. I'm guessing that the malware was Chinese, and that it originially targeted Chinese users.
...
That's the persnicious thing about the fact that it was installed as an IME.
It's not a normal excecutable that you can see in the process list; it's
