...
switching gears from defense to attack

[Last week][] we talked about how there are three ways to get code execution:
...
The main tool we'll be using is [WinDBG][], which is, no surprise, a windows debugger.
It is distributed by Microsoft as part of the free Debugging Tools for Windows package.

[WinDBG]: https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/

(Note: attach to *2nd* internet explorer process.)

We're going to focus this week on *turning a vulnerability into an exploit*.
...
heartbleed was not the same as a bog-standard buffer overrurn
buffer overruns are about a user-supplied value overrunning a fixed-size buffer, allowing us to write to values on the stack or heap
heartbleed was about the server reading a user-specfied amount of memory from a buffer, leaking memory to the internet
...
The main idea idea is that a use-after-free allows us to overwrite the freed
...
shellcode: malicious payload
calc.exe: a stand-in for a malicious program


vulnerability trigger - invokes bug to obtain control
payload - code run after control attained
    “shell code”
    etymology - payload usually ran a unix shell
    modern version: run calc.exe

memory corrucption -accessing memory in an invalid way which results in undefined behaviour
...
If you don't know what registers are, you're going to have a hard time debugging,
but in case not, here are a couple mnemonics.

        eax/ax/ah/al - return value of function
